...
public class Ex01_class {
	...
	//				 첫글자 대문자 나머지는 소문자
	//				 두 단어 이상일떄는 단어의 첫글자들은 대문자
	//				 숫자는 중간이나 뒤에 사용, 공백 문자는 사용 불가
	
	// 2. 맴버필드 : 데이터, 상태값, 속성, 특징
	//	  1) 변수 : 언제든지 변할 수 잇는 데이터를 저장하는 공간 
	...
	//	  				  생략 가능하고 여러개 사용 가능
	// 4) ** 오버로딕(중복정의) : 메서드 이름이 같으나 인자 수나 인자 자료형이 다르면 다른 메서드로 판단한다.
	...
	// 	  ** 모든 클래스는 반드시 생성자를 가지고 있습니다.
	//       해당 클래스의 생성자가 없으면 객체를 만들때 기본 생성자가 생략되어 있음.
	//		 해당 클래스의 생성작 없으면 개게를 생성할 때 기본 생성자를 호출 한다.
	...
	//  1) instance(인스턴스) 필드, 메서드 : 객체가 생성될 떄 같이 생성되는 필드와 메서드 
